...
for ((i=1;i <= ${ARRAY[0]};i++)); do
	...
	load=${ARRAY[$(echo $i*2-1|bc)]}
	moist=${ARRAY[$(echo $i*2|bc)]}
	if [ $i -lt 10 ]; then 
		tag=00$i
	elif [ $i -lt 100 ]; then  
			tag=0$i;
	...
	echo '|fuel load=' $load 
	echo '|fuel moisture=' $moist
	
	#Rewrite RunSet.in fuel load and moisture and ignMap file name
	sed '37 s/.*/'$load'/' RunSet.in >RunSet_tmp.in 
	sed '32 s/.*/'$moist'/' RunSet_tmp.in >RunSet_tmp2.in
