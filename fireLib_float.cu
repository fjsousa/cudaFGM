///Flavio Edit//////////////////////////////////////////////////////////////////
//
//	This file is the adaptation of fireLib.c to avoid troubles with CUDA 
//  (All cuda code will have no floats)		
//	
//  List of modifications:
//  
//            sometime in 2011: - all doubles are now floats
// 															- Function type EXTERN was removed - nvcc didn't 
//																liked it
//															- abs, pow functions became fabs and powf
 ...
 *
 *  Legalities
 *      fireLib - Copyright (C) 1996-2004 by Collin D. Bevins.  All rights reserved.
 *
 *  Naming Conventions
...
Fire_FuelCombustion ( FuelCatalogPtr catalog, size_t model )
{
    ...
    float sizeClassAreaWtg[FIRE_LIFE_CATS][FIRE_SIZE_CLASSES];
    float lifeLoad[FIRE_LIFE_CATS];
    float lifeArea[FIRE_LIFE_CATS];
    float lifeSavr[FIRE_LIFE_CATS];
    float lifeHeat[FIRE_LIFE_CATS];
    float lifeSeff[FIRE_LIFE_CATS];
    float lifeEtaS[FIRE_LIFE_CATS];
    ...
    for ( life=0; life<FIRE_LIFE_CATS; life++ )
    {
        ...
        if (lifeSeff[life] > 0.)
        {
            if ( (lifeEtaS[life] = 0.174 / powf(lifeSeff[life], 0.19)) > 1.0 )
                lifeEtaS[life] = 1.0;
        }
    ...
    /* Gamma */
    betaOpt   = 3.348 / (powf(sigma, 0.8189));
    ratio     = beta / betaOpt;
    aa        = 133. / (powf(sigma, 0.7913));
    sigma15   = powf(sigma, 1.5);
    gammaMax  = sigma15 / (495. + 0.0594*sigma15);
    gamma     = gammaMax * powf(ratio, aa) * exp(aa * (1.-ratio));
    ...
    Fuel_SlopeK(catalog,model) = 5.275 * powf(beta, -0.3);
    Fuel_WindB(catalog,model)  = 0.02526 * powf(sigma, 0.54);

    c = 7.47 * exp(-0.133 * powf(sigma, 0.55));
    e = 0.715 * exp(-0.000359 * sigma);
    Fuel_WindK(catalog,model) = c * powf(ratio, -e);
    Fuel_WindE(catalog,model) = powf(ratio, e) / c;
...
Fire_SpreadNoWindNoSlope ( 
    ...
    float  moisture[FIRE_MCLASSES]) 	/* array of fuel moistures   (fractions) */
{
    ...
    float qig;
    float ratio;
    float lifeMoisture[FIRE_LIFE_CATS];
    float lifeEtaM[FIRE_LIFE_CATS];
    float lifeMext[FIRE_LIFE_CATS];
...
Fire_SpreadWindSlopeMax ( 
    ...
    float  windDeg,            /* wind bearing vector      (compass degrees) */
    float  slope,              /* slope                         (rise/reach) */
    float  aspect)             /* aspect (downslope)   azimuth (compass deg) */
{
    float upslope, azimuthMax, phiEw;
    float splitDeg, splitRad;
    float slpRate, wndRate, rv, spreadMax;
    float x, y, al, a;
    float maxWind, effectiveWind, lwRatio, eccentricity, backing;
    ...
    if ( ! Equal(Fuel_WindSpeed(catalog,model),windFpm) )
    {
        Fuel_PhiWind(catalog,model) = (windFpm < Smidgen) ? 0. :
            Fuel_WindK(catalog,model) * powf(windFpm, Fuel_WindB(catalog,model));
    ...
    if ( doEffectiveWind )
        effectiveWind = powf( (phiEw * Fuel_WindE(catalog,model)),
    ...
    if ( checkWindLimit )
    {
        ...
        if ( effectiveWind > maxWind )
        {
            phiEw = (maxWind < Smidgen) ? 0. :
                Fuel_WindK(catalog,model) * powf(maxWind, Fuel_WindB(catalog,model));
...
Fire_SpreadAtAzimuth ( 
    FuelCatalogPtr catalog,     /* FuelCatalogData instance pointer           */
    size_t  model,              /* fuel model number            [0-maxModels] */
    float  azimuth,            	/* fire spread azimuth     (deg from upslope) */
    ...
    if ( which )
    {
        ...
        if ( (which & FIRE_FLAME) )
        {
            ...
            else
            {
                ...
                {
                    Fuel_FlameLength(catalog,model) = 0.45 * powf(byrams, 0.46);
                }
            }
        }
        ...
        if ( (which & FIRE_SCORCH) )
        {
            ...
            else
            {
                ...
                Fuel_ScorchHeight(catalog,model) =
                    powf(byrams, 1.166667) / sqrt(byrams + (mph * mph * mph));
