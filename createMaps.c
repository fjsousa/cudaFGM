...
//	createMaps.c
// 
//  An executable is created to creat aspect and slope maps. It can create maps 
//	for two constant condition scenarios (test cases) and any map from a input 
//  height map. The input height map is scaled to the desired number of rows 
//	and cols and then the aspect and slope maps are computed. 
// 
//  Algorithm used is shit and should be modified to least squares eventually.
//
// Inputs: "RunSet.in" file - Rows, Cols, Height, Width.
//         The file "input.map" must be present in the same folder.
//
// Outputs: "reSized.map" is the resized height map.
//					"aspect.map" and "slope.map"
////////////////////////////////////////////////////////////////////////////////
...
int main (void)
{
	...
	double W, H;   				//Width and Height (Read in meters)
	double* reSized;			//Pointer to resized map
	...
	for (n = 0; n < 24; n++)
		fgets(buffer, 100, IN);
		//Reads setup data
		fscanf(IN, "%lf %lf %d %d", &W, &H, &N, &M);

	reSized = F_reSize(N, M, MetersToFeet(W), MetersToFeet(H));	
	F_SlopeAspect(reSized, N, M, MetersToFeet(W), MetersToFeet(H));
...
double* F_reSize(int N, int M, double W, double H)
{
	...
	for (row = 0; row < n; row++)
	{
		for (col = 0; col < m; col++)
		{
			...
			//original[col + m*row] *=0.4; //acho que e aqui que esta a escalar
		}
	}
